...
(() => {
  ...
  // content.js — robust “PDF present” detector for HTML wrappers
(function detectPDF() {
  const PDF_SELECTORS = [
  ...
  function hasPDF() {
    // 1) obvious cases via DOM
    if (PDF_SELECTORS.some(sel => document.querySelector(sel))) return true;

    // 2) PDFs sometimes injected late; check common viewer container ids/classes
    const hint = document.querySelector('#pdf, .pdf-viewer, [data-pdf], [id*="pdf"], [class*="pdf"]');
    if (hint) return true;

    // 3) last resort: look for an <embed> with PDF extension in src
  ...
  function notify(status) {
    try {
      chrome.runtime.sendMessage({ type: 'PDF_DETECTED', present: status, url: location.href });
    } catch {}
  }
